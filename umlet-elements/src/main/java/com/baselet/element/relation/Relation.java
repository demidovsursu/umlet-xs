...
public class Relation extends NewGridElement implements Stickable, RelationPointHolder {
	...
	protected void resetAndDrawMetaDrawerContent(DrawHandler drawer) {
		...
		drawer.setForegroundColor(isActive()?ColorOwn.ACTIVE_FG:ColorOwn.SELECTION_FG);
	...
	public String getLineType() {
		String s=state.getFacetResponse(RelationLineTypeFacet.class, null);
		if(s==null) return "-";
		return s;
	}
	public PointDoubleIndexed getPointN(int n) {
		List<PointDoubleIndexed> sp=(List<PointDoubleIndexed>)getStickablePoints();
		String lt=getLineType();
		if(lt.charAt(0)=='-' || lt.charAt(0)=='.')
			return sp.get(n);
		else			
			return sp.get(1-n);
	}
