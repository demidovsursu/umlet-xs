...
public class RelationDrawer {
	...
	public static enum ArrowEndType {
		NORMAL, CLOSED, DIAMOND, MEASURE, CROSSED
	}

	public static void drawArrowToLine(DrawHandler drawer, Line line, boolean drawOnStart, ArrowEndType arrowEndType, boolean fillBody, boolean invertArrow, int crowMode) {
		drawArrowToLine(line.getPointOnLineWithDistanceFrom(drawOnStart, ARROWEND_DISPLACEMENT), drawer, line, drawOnStart, arrowEndType, fillBody, invertArrow, crowMode);
	}
	public static void drawArrowToLine(DrawHandler drawer, Line line, boolean drawOnStart, ArrowEndType arrowEndType, boolean fillBody, boolean invertArrow) {
		drawArrowToLine(line.getPointOnLineWithDistanceFrom(drawOnStart, ARROWEND_DISPLACEMENT), drawer, line, drawOnStart, arrowEndType, fillBody, invertArrow, -1);
	}
	public static void drawArrowToLine(PointDouble point, DrawHandler drawer, Line line, boolean drawOnStart, ArrowEndType arrowEndType, boolean fillBody, boolean invertArrow) {
		drawArrowToLine(point, drawer, line, drawOnStart, arrowEndType, fillBody, invertArrow, -1);
        }
	public static void drawArrowToLine(PointDouble point, DrawHandler drawer, Line line, boolean drawOnStart, ArrowEndType arrowEndType, boolean fillBody, boolean invertArrow, int crowMode) {
		int arrowAngle = drawOnStart?150:30;
		if (invertArrow) {
			point = line.getPointOnLineWithDistanceFrom(drawOnStart, ARROW_LENGTH);
			arrowAngle = drawOnStart?30:150;
		}
		...
		PointDouble p2 = calcPointArrow(point, line.getAngleOfSlope() + arrowAngle);
                if(arrowEndType == ArrowEndType.CROSSED) {
                	point = line.getPointOnLineWithDistanceFrom(drawOnStart, ARROW_LENGTH/2);
		        p1 = calcPoint(point, line.getAngleOfSlope() - 90, ARROW_LENGTH/2);
	                p2 = calcPoint(point, line.getAngleOfSlope() + 90, ARROW_LENGTH/2);
                }
		List<PointDouble> points = new ArrayList<PointDouble>(Arrays.asList(p1));
                if(arrowEndType != ArrowEndType.CROSSED) {
                	points.add(point);
                }
		...
		if (arrowEndType == ArrowEndType.CLOSED) {
			...
			PointDouble pDiamond = line.getPointOnLineWithDistanceFrom(drawOnStart, lengthDiamond);
			points.add(pDiamond);
			points.add(p1);
		}
		...
		if (fillBody) {
			...
			drawer.drawLines(points);
                ...
                if(crowMode == 0) {
			point = line.getPointOnLineWithDistanceFrom(drawOnStart, ARROW_LENGTH*5/4);
			drawer.drawCircle(point.getX(), point.getY(), ARROW_LENGTH/4);
                }
                else if(crowMode == 1) {
			point = line.getPointOnLineWithDistanceFrom(drawOnStart, ARROW_LENGTH);
			drawer.drawLines(
		        	calcPoint(point, line.getAngleOfSlope() - 90, ARROW_LENGTH/2),
	                	calcPoint(point, line.getAngleOfSlope() + 90, ARROW_LENGTH/2));
                }
	}
