...
public class SpecialStateTypeFacet extends KeyValueFacet {
	...
	public void handleValue(final String value, final PropertiesParserState state) {
		...
		String act=state.getFacetResponse(ActiveFacet.class, null);
		boolean isActive=!(act==null || act.length()==0 || act.equals("0"));
		...
		if (type == StateTypeEnum.INITIAL) {
			drawBlackEllipse(drawer, w - 1, h - 1, 1, isActive);
		}
...
//			drawer.setForegroundColor(ColorOwn.TRANSPARENT); // don't use foregroundcolor for the inner circle, because otherwise in Swing it would look very ugly
			...
			drawer.setLineWidth(0.25);
			double ellipseDistance = Math.max(w - 1, h - 1) / 5.5;
			drawBlackEllipse(drawer, w - ellipseDistance * 2, h - ellipseDistance * 2, ellipseDistance, isActive);
//			drawer.setForegroundColor(oldFg);
	...
	private void drawBlackEllipse(final DrawHandler drawer, double width, double height, double xY, boolean isActive) {
		ColorOwn oldBg = drawer.getBackgroundColor();
		if (drawer.getBackgroundColor() == ColorOwn.DEFAULT_BACKGROUND) {
			drawer.setBackgroundColor(isActive?ColorOwn.ACTIVE_FG:ColorOwn.BLACK.transparency(Transparency.FOREGROUND));
		}
		else {
