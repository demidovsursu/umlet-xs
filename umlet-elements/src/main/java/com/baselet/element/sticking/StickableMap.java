...
public class StickableMap {
	...
	public List<Stickable> getStartStickables() {
		...
		for (Entry<Stickable, List<Integer>> entry : stickingIndexMap.entrySet()) {
			if(entry.getKey() instanceof Relation) {
				Relation r=(Relation)(entry.getKey());
				List<Integer> s = entry.getValue();
				if(s!=null && s.contains(r.getPointN(0).getIndex()))
					returnList.add(r);
			}
		}
		return returnList;
	...
	public List<Stickable> getEndStickables() {
		...
		for (Entry<Stickable, List<Integer>> entry : stickingIndexMap.entrySet()) {
			if(entry.getKey() instanceof Relation) {
				Relation r=(Relation)(entry.getKey());
				List<Integer> s = entry.getValue();
				if(s!=null && s.contains(r.getPointN(1).getIndex()))
					returnList.add(r);
			}
		}
		return returnList;
	...
	public boolean isStickableEnds(Stickable r) {
		List<Integer> points = stickingIndexMap.get(r);
		if (points == null) return false;
		return points.contains(((Relation)r).getPointN(1).getIndex());
	}
