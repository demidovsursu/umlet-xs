...
*/
	public List<Relation> getStickables(Collection<GridElement> excludeList) {
		if (!SharedConfig.getInstance().isStickingEnabled() || handler instanceof PaletteHandler) {
			return Collections.<Relation> emptyList();
		}
		List<Relation> returnList = getHelper(Relation.class);
	...
	public Point getViewPosition() {
		return _scr.getViewport().getViewPosition();
	}
	public void setViewPosition(Point p) {
		_scr.getViewport().setViewSize(getPreferredSize());
		_scr.getViewport().setViewPosition(p);
	}
	...
	public GridElement getElementOnEnd(GridElement r) {
		if(!(r instanceof Relation)) return null;
		for (GridElement e : gridElements) {
			if (e instanceof NewGridElement && ((NewGridElement)e).isStickableEnds((Relation)r)) {
				return e;
			}
		}
		return null;
	}
	public List<GridElement> getActiveElements() {
		List<GridElement> gridElementsToReturn = new ArrayList<GridElement>();
		for (GridElement e : getGridElements()) {
			if (e instanceof NewGridElement && ((NewGridElement)e).isActive()) {
