...
public class DrawHandlerSwing extends DrawHandler {
	...
	public void drawImage(String name, double x, double y, double width, double height) {
		String fullname=handler.getFullPathName();
		String dir="";
		if(fullname!=null) {
		...
		Image img0=null;
		try {
			img0=Toolkit.getDefaultToolkit().getImage(dir+name);
		} catch(Throwable t) { img0=null; }
		if(img0==null) return;
		final Image img=img0;
		final int xZoomed = (int)(x * getZoom());
		final int yZoomed = (int)(y * getZoom());
		final int wZoomed = (int)(width * getZoom());
		final int hZoomed = (int)(height * getZoom());
		addDrawable(new DrawFunction() {
			@Override
			public void run() {
				g2.drawImage(img, xZoomed, yZoomed, wZoomed, hZoomed, null);
			}
		});
	}
	@Override
