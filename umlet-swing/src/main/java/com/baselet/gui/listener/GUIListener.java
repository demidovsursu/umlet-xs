		...
		if (handler != null && !e.isAltDown() && !e.isAltGraphDown() /* && !e.isControlDown() && !e.isMetaDown() */) {
			 ...
			 */
				...
				if(ch!=KeyEvent.CHAR_UNDEFINED && ch>=' ') 
					keypress=String.valueOf(ch);
				else
					keypress=KeyEvent.getKeyText(e.getKeyCode());
				Command c=handler.emulationKey(keypress);
				if(c!=null) handler.getController().executeCommand(c);
			}
		}
